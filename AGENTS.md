...
Outside Shipping Mode, a worker receiving the alert must stop before another write, merge/pull `main`, resolve only owned-path conflicts, rerun affected fast checks, and only then resume. During Shipping Mode, the alert is a mandatory sync boundary before submission/merge/dependency consumption rather than an immediate idle barrier for an independent in-progress owned-path batch; that lane must synchronize the latest required green `ship/week-1` and rerun affected fast checks before it submits or consumes the merged contract. Chat-only completion/alert messages are not durable repository evidence; PRs and the broadcast issue are the persistent coordination surfaces.
