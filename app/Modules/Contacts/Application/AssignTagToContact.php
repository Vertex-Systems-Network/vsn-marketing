...
final readonly class AssignTagToContact
{
    public const AUDIT_ACTION = 'contact.tag.assigned';
    ...
    public function handle(TenantContext $context, string $tagId, string $contactId): bool
    {
        return $this->transaction->run(function () use ($context, $tagId, $contactId): bool {
            ...
            $changed = $this->tags->assignContact($context->workspaceId, $tagId, $contactId);
            if (! $changed) {
                return false;
            }
