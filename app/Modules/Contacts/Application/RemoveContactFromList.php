...
final readonly class RemoveContactFromList
{
    public const AUDIT_ACTION = 'contact_list.membership.removed';
    ...
    public function handle(TenantContext $context, string $listId, string $contactId): bool
    {
        return $this->transaction->run(function () use ($context, $listId, $contactId): bool {
            ...
            $changed = $this->lists->removeContact($context->workspaceId, $listId, $contactId);
            if (! $changed) {
                return false;
            }
