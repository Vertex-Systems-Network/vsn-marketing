...
final class GmailErrorNormalizer implements ProviderErrorNormalizer
{
    ...
    private function categoryFor(ProviderFailureEvidence $failure): ProviderErrorCategory
    {
        ...
        if ($status === 429 || in_array($code, ['ratelimitexceeded', 'userratelimitexceeded', 'quotaexceeded'], true)) {
            return ProviderErrorCategory::RateLimited;
        }

        if ($status === 401 || in_array($code, ['autherror', 'invalidcredentials'], true)) {
            return ProviderErrorCategory::Authentication;
        }

        if ($status === 403 || in_array($code, ['insufficientpermissions', 'forbidden'], true)) {
        ...
        if ($status === 400 || in_array($code, ['invalidargument', 'failedprecondition'], true)) {
